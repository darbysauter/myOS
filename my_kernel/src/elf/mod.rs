...
pub fn get_global_offset_table(boot_info: &BootInfo) -> &mut [usize] {
    ...
    let sh_off: usize =
        u64::from_le_bytes(
            e.get(0x28..0x30).expect("Couldn't get offset [0]")
            .try_into().expect("Couldn't get offset [1]"))
            .try_into().expect("Couldn't get offset [2]");

    let sh_ent_size: u16 =
        u16::from_le_bytes(
            e.get(0x3a..0x3c).expect("Couldn't get ent size [0]")
            .try_into().expect("Couldn't get ent size [1]"))
            .try_into().expect("Couldn't get ent size [2]");
    ...
    let sh_ent_num: u16 =
        u16::from_le_bytes(
            e.get(0x3c..0x3e).expect("Couldn't get ent num [0]")
            .try_into().expect("Couldn't get ent num [1]"))
            .try_into().expect("Couldn't get ent num [2]");


    let sh_str_index: u16 =
    u16::from_le_bytes(
        e.get(0x3e..0x40).expect("Couldn't get ent num [0]")
        .try_into().expect("Couldn't get ent num [1]"))
        .try_into().expect("Couldn't get ent num [2]");
